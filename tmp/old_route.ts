...
export async function GET(
  ...
  if (cached && (now - cached.timestamp < CACHE_TTL)) {
    console.log(`≡ƒÜÇ Cache Hit: MAL_ID=${id}, EP=${episode}`);
    return NextResponse.json(cached.data);
  }

  console.log(`≡ƒôí Stream Request: MAL_ID=${id}, EP=${episode}, Title=${title}, TitleEn=${titleEn}, Year=${queryYear}`);
  ...
  try {
    ...
    console.log(`≡ƒöì AGGREGATOR: Starting vast provider search...`);
    ...
    for (const searchQuery of searchQueries) {
      ...
      for (const p of providers) {
        try {
          ...
          console.warn(`ΓÜá∩╕Å Provider ${p.name} search failed for "${queryToUse}":`, err);
        }
      }
    ...
    if (bestMatch && winningProvider) {
      console.log(`≡ƒÅå Winner: [${winningProvider.name}] ${bestMatch.title} (ID: ${bestMatch.id}) Score: ${highestScore}`);
      ...
      if (targetEp) {
        console.log(`≡ƒÄ¼ Fetching sources from ${winningProvider.name} for episode ${epNum}...`);
    ...
    // 2. Python Fallback
    console.log("≡ƒÉì Falling back to Python Gogoanime Extractor...");
    ...
    console.error('Γ¥î Aggregator failed:', error);
